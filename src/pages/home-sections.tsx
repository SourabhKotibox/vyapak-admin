...
export default function HomeSections() {
  ...
  const { data: sectionsData, isLoading } = useGetSections({ contentType: 'drama' });
  ...
  const activeTabName = tabsConfig.find((t: any) => t.id === 'drama')?.name || 'Short Dramas';

  const handleSaveTabName = () => {
    if (!tabNameInput.trim()) return;
    const newTabs = tabsConfig.map((t: any) => t.id === 'drama' ? { ...t, name: tabNameInput } : t);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border-border rounded-xl p-4">
        {isLoading ? (
          <div className="text-center py-10 text-foreground/65">Loading layout...</div>
              ...
              {(formData.contentSelection === 'manual' || formData.contentSelection === 'mixed') && (
                <div className="col-span-2 space-y-3 pt-4 border-t border-border mt-2">
                  ...
                  <Select onValueChange={(v) => {
                    ...
                    <SelectContent>
                       {(dramasRes?.data || []).filter((i:any) => i.contentType === 'drama').map((item: any) => (
                         <SelectItem key={item._id} value={item._id}>
                           {item.title} (Short Drama)
                         </SelectItem>
                       ))}
                    </SelectContent>
                  </Select>
                  ...
                  {selectedItems.length > 0 && (
                    <div className="flex flex-col gap-2 mt-3 bg-muted/30 dark:bg-black/20 border border-border dark:border-white/5 p-2 rounded-md max-h-48 overflow-y-auto">
                      {selectedItems.map(id => {
                        const allItems = (dramasRes?.data || []).filter((i:any) => i.contentType === 'drama');
