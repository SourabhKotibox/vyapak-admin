                    ...
                    <SelectTrigger className="bg-muted border-border">
                     ...
                     <SelectContent>
                       {dramasRes?.data?.filter((i:any) => i.contentType === 'drama').map((item: any) => (
                         <SelectItem key={item._id} value={item._id}>[Short Drama] {item.title}</SelectItem>
                       ))}
                     </SelectContent>
                  </Select>
                  ...
                  {selectedItems.length > 0 && (
                    <div className="flex flex-col gap-2 mt-3 bg-muted/30 dark:bg-black/20 border border-border dark:border-white/5 p-2 rounded-md max-h-48 overflow-y-auto">
                      {selectedItems.map(id => {
                        const matchedItem = dramasRes?.data?.find((i: any) => i._id === id);
                        return (
                          <div key={id} className="flex items-center justify-between bg-muted/50 px-3 py-2 rounded-md text-sm border border-border/50">
                            <span className="truncate pr-4">{matchedItem ? '[Short Drama] ' + matchedItem.title : 'Unknown Title'}</span>
                            <Button 
                              variant="ghost" 
                              size="icon" 
